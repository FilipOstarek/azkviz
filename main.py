...
class Sestiuhelnik(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.orandzova = False
        self.modra = False
        self.prechod = 0  # podminka pro konec
        # self.barva = (255,0,0)
        # důležité pro souradnce
        self.oznaceni = para.bc
        self.cislo = [para.souradnice, para.rada]
        self.souradnice = para.souradnice
        self.rada = para.rada
        ...
        for entity in sestiuhelniky:
            if (pos[0] >= self.bod6[0] and pos[0] <= self.bod2[0]) and pos[1] >= self.bod1[1] and pos[1] <= self.bod4[
                ...
                if para.zacinajici_hrac == True:
                    ...
                    hracO.update(self.podminka[0], self.podminka[1], self.podminka[2], self.cislo)
                ...
                if para.zacinajici_hrac == False:
                    ...
                    hracM.update(self.podminka[0], self.podminka[1], self.podminka[2], self.cislo)
...
class O_hrac(pygame.sprite.Sprite):  # orandzovy hrac
    def __init__(self):
        ...
        self.dolni_s = False
        self.levaSOURADNICE = []
        self.pravaSOURADNICE = []
        self.dolniSOURADNICE = []
        self.bod1 = para.bod1
        self.bod2 = para.bod2
        self.bod3 = para.bod3
        self.bod4 = para.bod4
    ...
    def update(self, leva, dolni, prava, cislo):
        pass
...
class Otazka(pygame.sprite.Sprite):
    def __init__(self):
        ...
        '''
        self.bod1 = (para.obrazovka_x - ((para.vrchol_x / 10) * 19),
                     ((para.vrchol_y + 2 * para.velikost + para.odsazeni_y * para.zadano) + para.velikost * 2))
        self.bod2 = ((para.obrazovka_x - ((para.vrchol_x / 2.75 + para.velikost * 10 + para.vrchol_x - para.mezera))),
                     ((para.vrchol_y + 2 * para.velikost + para.odsazeni_y * para.zadano) + para.velikost * 3))
        self.bod3 = ((para.vrchol_x / 2.75 + para.velikost * 10 + para.vrchol_x - para.mezera),
                     ((para.vrchol_y + 2 * para.velikost + para.odsazeni_y * para.zadano) + para.velikost * 3))
        self.bod4 = (((para.vrchol_x / 10) * 19),
                     ((para.vrchol_y + 2 * para.velikost + para.odsazeni_y * para.zadano) + para.velikost * 2))
        self.bod5 = ((para.vrchol_x / 2.75 + para.velikost * 10 + para.vrchol_x - para.mezera),
                     ((para.vrchol_y + 2 * para.velikost + para.odsazeni_y * para.zadano) + para.velikost))
        self.bod6 = ((para.vrchol_x / 6 - para.mezera),
                     ((para.vrchol_y + 2 * para.velikost + para.odsazeni_y * para.zadano) + para.velikost))
...
class Odpovedi(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.bod1 = (para.obrazovka_x - ((para.vrchol_x / 10) * 19) + (self.umisteni * ((para.vrchol_x / 10) * 9.5)), (
                    (para.vrchol_y + 2 * para.velikost + para.odsazeni_y * para.zadano) + para.velikost * 2 * 2.5 + (
                        self.rada * (para.velikost * 3))))
        self.bod2 = ((para.obrazovka_x - ((para.vrchol_x / 2.75 + para.velikost * 10 + para.vrchol_x - para.mezera)) + (
                    self.umisteni * ((para.vrchol_x / 10) * 9.5))), ((
                                                                                 para.vrchol_y + 2 * para.velikost + para.odsazeni_y * para.zadano) + para.velikost * 3 * 2 + (
                                                                                 self.rada * (para.velikost * 3))))
        self.bod3 = (((para.vrchol_x / 2.75 + para.velikost * 10 + para.vrchol_x - para.mezera) / 2 - para.mezera + (
                    self.umisteni * ((para.vrchol_x / 10) * 9.5))), ((
                                                                                 para.vrchol_y + 2 * para.velikost + para.odsazeni_y * para.zadano) + para.velikost * 3 * 2 + (
                                                                                 self.rada * (para.velikost * 3))))
        self.bod4 = ((((para.vrchol_x / 10) * 19) / 2 + (self.umisteni * ((para.vrchol_x / 10) * 9.5))), (
                    (para.vrchol_y + 2 * para.velikost + para.odsazeni_y * para.zadano) + para.velikost * 2 * 2.5 + (
                        self.rada * (para.velikost * 3))))
        self.bod5 = (((para.vrchol_x / 2.75 + para.velikost * 10 + para.vrchol_x - para.mezera) / 2 - para.mezera + (
                    self.umisteni * ((para.vrchol_x / 10) * 9.5))), ((
                                                                                 para.vrchol_y + 2 * para.velikost + para.odsazeni_y * para.zadano) + para.velikost * 4 + (
                                                                                 self.rada * (para.velikost * 3))))
        self.bod6 = ((para.vrchol_x / 6 - para.mezera + (self.umisteni * ((para.vrchol_x / 10) * 9.5))), (
                    (para.vrchol_y + 2 * para.velikost + para.odsazeni_y * para.zadano) + para.velikost * 4 + (
                        self.rada * (para.velikost * 3))))
        # print(self.sou)
...
while para.running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key == para.K_ESCAPE:
                para.running = False
        elif event.type == pygame.MOUSEBUTTONDOWN and para.vitezM != True and para.vitezO != True:
            cas.tick(2000)
            # print("click")
    ...
    if para.zacatek == True:
        barva3 = para.sed
    # Otázka
    pygame.draw.polygon(para.obrazovka, barva3, (
    pole_otazky.bod1, pole_otazky.bod2, pole_otazky.bod3, pole_otazky.bod4, pole_otazky.bod5, pole_otazky.bod6))
    # Odpovědi
    pygame.draw.polygon(para.obrazovka, barva3,
                        (odpoved1.bod1, odpoved1.bod2, odpoved1.bod3, odpoved1.bod4, odpoved1.bod5, odpoved1.bod6))
    pygame.draw.polygon(para.obrazovka, barva3,
                        (odpoved2.bod1, odpoved2.bod2, odpoved2.bod3, odpoved2.bod4, odpoved2.bod5, odpoved2.bod6))
    pygame.draw.polygon(para.obrazovka, barva3,
                        (odpoved3.bod1, odpoved3.bod2, odpoved3.bod3, odpoved3.bod4, odpoved3.bod5, odpoved3.bod6))
    pygame.draw.polygon(para.obrazovka, barva3,
                        (odpoved4.bod1, odpoved4.bod2, odpoved4.bod3, odpoved4.bod4, odpoved4.bod5, odpoved4.bod6))
    # pygame.display.flip()
    if para.vitezM:
